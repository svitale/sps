...
function crForm($id_subject,$key) {
	...
	$date_end = "2009-09-01";
	$out =  '<div style="width:612px; background-color: lightgrey";>';
	$query = "SELECT count(*) as parentcount,id_parent,id_subject,date_collection,batch_quality.sample_type,batch_quality.shipment_type,batch_quality.id_visit,batch_quality.family,crf.quantity,crf.`num_order` FROM `batch_quality` left join crf on (batch_quality.family = crf.family and batch_quality.shipment_type = crf.shipment_type and batch_quality. sample_type = crf.sample_type) WHERE `date_receipt` = '0000-00-00' and id_subject = '$id_subject' ";
	if (($date_start > "0") && ($date_end > "0")) {
	$query .= " and batch_quality.date_ship > '$date_start' and batch_quality.date_ship < '$date_end' ";
	}
	if (isset($_SESSION['shipment_type'])) {
	$query .= ' and batch_quality.shipment_type = \''.$_SESSION['shipment_type'].'\'';
	}
	$query .= " group by id_subject,sample_type,shipment_type,id_visit,id_parent,batch_quality.family,batch_quality.shipment_type order by id_subject,crf.num_order,sample_type,shipment_type,id_visit,id_parent DESC;";
	...
	if (isset($_SESSION['quantity']{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family})) {
        $quantity =  ($_SESSION['quantity']{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family});
	}
	if ($id_parent == '0') {
	$class = 'crfparent';
	} else {
	$class = 'crfchild';
	$count{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family} = $parentcount; 
	} 



if ($id_subject != $last_subject) {
	$out .=  '<div style="width:420px">';
	...
	$out .= '<div class="'.$class.'" id="shipment_type_'.$key.'_'.$j.'"  style="width: 120px;">'.$shipment_type.'</div>';
if (isset($count{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family})) {
	$out .= '<div class="'.$class.'" id="quantity_'.$key.'_'.$j.'"  style="width: 100px; background-color: lightgreen"">0</div>';
	$out .= '<div class="'.$class.'" id="alqs_'.$key.'_'.$j.'"  style="width: 38px; background-color: lightgrey"">'.$count{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family}.'</div>';
        $_SESSION['quantity']{$id_subject}{$sample_type}{$shipment_type}{$id_visit}{$family} = 0;
} else {
