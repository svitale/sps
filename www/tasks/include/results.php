	...
	if (isset($_SESSION['tmptable'])) {
		...
		if (mysql_num_rows(mysql_query($query)) > 0) {
			...
			while($row = mysql_fetch_assoc($result)) {
				...
				echo '<input type="button" onclick="window.open(\'npc.php?action=data&format=csv&type=import\')" value="export records">';
			}
		} else {
			echo "failed";
		}
...
function importBatch() {
	global $id_study;
	$tmptable = $_SESSION['tmptable'];
        if (isset($_SESSION['dsttable'])) {
	    $dsttable = $_SESSION['dsttable'];
        } else {
	    $dsttable = null;
        }
	if ($_SESSION['task'] == 'orders') {
	...
	} else if ($dsttable == 'results_raw') {
		$referrer = "?task=analysis&id_rungroup=".urlencode($_SESSION['id_rungroup']);
		$query = "insert into results_raw (id_assay,id_barcode,id_uuid,id_subject,id_study,id_lab,id_instrument,id_visit,sample_type,shipment_type,date_collection,date_visit,id_rungroup,id_retest,name_plate,position_plate,layout_plate,barcode_source,position_source,value,value_1,value_2,cv,wavelength,dilution,value_measured,value_calculated,units,units_measured,units_calculated,datetime_assay,timestamp,qc,uqc,reviewed,calibrator,reagent,cleaner,`ignore`,`share`,destination,volumestatus,notes) (select id_assay,id_barcode,id_uuid,id_subject,id_study,id_lab,id_instrument,id_visit,sample_type,shipment_type,date_collection,date_visit,id_rungroup,id_retest,name_plate,position_plate,layout_plate,barcode_source,position_source,value,value_1,value_2,cv,wavelength,dilution,value_measured,value_calculated,units,units_measured,units_calculated,datetime_assay,timestamp,qc,uqc,reviewed,calibrator,reagent,cleaner,`ignore`,`share`,destination,volumestatus,notes from  `".$tmptable."`);";
	} else {
		$referrer = "?task=results&id_rungroup=".urlencode($_SESSION['id_rungroup']);
		$query = "insert into results (id_barcode,id_uuid,id_subject,id_study,id_lab,id_instrument,id_assay,id_visit,id_rungroup,id_retest,value,units,datetime_assay,date_collection,date_visit,timestamp,qc,uqc,reviewed,calibrator,reagent,cleaner,`ignore`,share,notes,sample_type,shipment_type) (select id_barcode,id_uuid,id_subject,id_study,id_lab,id_instrument,id_assay,id_visit,id_rungroup,id_retest,value,units,datetime_assay,date_collection,date_visit,timestamp,qc,uqc,reviewed,calibrator,reagent,cleaner,`ignore`,share,notes,sample_type,shipment_type from `".$tmptable."`);";
	}
	$result = mysql_query($query);
        if(!$result) {
...
function data_array($type) {
	...
	if (isset($_SESSION['tmptable'])) {
		...
		$sql = "select `$table`.id_rungroup,`$table`.id_retest,`$table`.id_barcode,`$table`.uqc,`$table`.id_subject,`$table`.id_visit,`$table`.sample_type,`$table`.id_study,`$table`.id_instrument,`$table`.id_assay,`$table`.value,`$table`.cv";
		...
		if ((!isset($_SESSION['filters']) || (count($_SESSION['filters']) == 0)) && (!isset($_SESSION['id_rungroup']))) {
		...
		$returnArray = array();
		$query = "select `$table`.reviewed,`$table`.share as approved,`$table`.`ignore`,`$table`.id_rungroup,`$table`.id_retest,`$table`.id,`$table`.id_barcode,`$table`.id_subject,`$table`.id_visit,`$table`.id_study,`$table`.id_instrument,`$table`.id_assay,`$table`.value,`$table`.cv";
...
//			$params = retArrayParamsById($_SESSION['params'],array('id_study','id_assay','id_instrument','id_visit','id_lab'));
//			$array_fields = array_keys($params);
//			$query .= retQueryFilter($_SESSION['params'],$array_fields,$table) ;

$resultsObjects = new ResultsObjects();
$resultsObjects->table = 'results';
$resultsObjects->filters = $_SESSION['filters'];
			$query .= $resultsObjects->filterSql();
			$query .= " and id_study = '".$id_study."' ";
...
$returnArray = array();
//$i = 1;
$query = "select `$table`.reviewed,`$table`.share as approved,`$table`.id_retest as retest,`$table`.id_rungroup as rungroup,`$table`.id,`$table`.id_barcode,`$table`.id_subject,`$table`.id_visit,`$table`.sample_type,`$table`.id_study,`$table`.id_instrument,`$table`.id_lab,`$table`.id_assay,`$table`.`value` ,`$table`.units,`$table`.date_visit,`$table`.datetime_assay";
...
if (isset($_SESSION['check_dates']) && $_SESSION['check_dates'] == '1') {
		...
		$query .= " left join `$table` on (snapshots.id_results = `$table`.id) left join visits on (`$table`.id_subject = visits.id_subject and `$table`.id_visit = visits.id_visit )";
...
?>
	...
	<p><input type="submit" onclick="updateResult('<?php echo $id?>', {params:Form.serialize('myform') }); Modalbox.hide();" value="Submit Note"> 
	</fieldset>
	<br/>
</form>
