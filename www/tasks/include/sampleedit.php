   ...
   function operateScannedObject($table, $id) {
        ...
        switch ($action){
            case 'display':
                ...
                break;
            case 'history':
                $query = "(select l.id_item, l.id_container, items.id_uuid as uuid_contaner, l.freezer, l.subdiv1 as shelf, l.subdiv2 as rack, l.subdiv3 as box, subdiv4 as row, subdiv5 as col,
                l.timestamp as date_placed,
                if(date_moved, date_moved, 'current location') as date_moved, '' as name_created
                from locations_moved as l 
                inner join items on items.id = l.id_container
                where l.id_item = $id)
                union 
                (select l.id_item, l.id_container, items.id_uuid as uuid_container, l.freezer, l.subdiv1 as shelf, l.subdiv2 as rack, subdiv3 as box, subdiv4 as row, subdiv5 as col,
                l.timestamp as date_placed,
                if(date_moved, date_moved, 'current location') as date_moved, l.name_created
                from locations as l
                inner join items on items.id = l.id_container 
                where id_item = $id)
...
function SetTubeConsumed($tubeid, $value = true) {
    ...
    $statement = "update items set consumed = $valStr where id = $tubeid and type = 'tube'";
    echo $statement;
    $result = mysql_query($statement);
    if(!$result) {
        return false;
...
function SetBoxConsumed($boxid, $value = true) {
    ...
    while($row = mysql_fetch_array($tubeResult)) {
        $id_item = $row['id_item'];
        $result = mysql_query("update items set consumed = $valStr where type = 'tube' and id = $id_item");
        if($result == false){
...
function SetTubeThawed($tubeid, $value = true) {
    if($value)
        ...
        $valStr = "quant_thaws - 1";
    $statement = "update items set quant_thaws = $valStr where id = $tubeid and type = 'tube'";
//    echo $statement;
    $result = mysql_query($statement);
    if(!$result) {
        return false;
...
function SetBoxThawed($boxid, $value = true) {
    ...
    while($row = mysql_fetch_array($tubeResult)) {
        $id_item = $row['id_item'];
        $result = mysql_query("update items set quant_thaws = $valStr where type = 'tube' and id = $id_item");
        if($result == false){
...
function decrementBoxVolume($boxid, $variable) {
	...
	$id_item = $targetTube['id_item'];
        $sql = "update items set quant_cur = quant_cur - $decrementor where type = 'tube' and id = $id_item and quant_cur +0 > $decrementor";
...
function decrementTubeVolume($tubeid, $variable) {
    ...
    $sql = "update items set quant_cur = quant_cur - $decrementor where type = 'tube' and id = $tubeid and quant_cur +0 > $decrementor";
    $result = mysql_query($sql);
    if($result == false){
